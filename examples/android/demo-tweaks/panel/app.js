...
const elements = {
  ...
  toolSegments: document.querySelector("#tool-segments"),
...
function renderToolViews() {
  ...
  elements.toolSegments.hidden = views.length < 2;

  if (!app || views.length < 2) {
    elements.toolSegments.replaceChildren();
    return;
  }
  ...
  const segments = views.map((view) => {
    ...
    const button = node("button", "tool-segment");

    button.type = "button";
    button.disabled = state.switching;
    button.setAttribute("aria-label", `${title} tool`);
    button.setAttribute("aria-pressed", String(selected));
    button.setAttribute("title", title);
    button.append(mockIcon(view === "network" ? "network" : "tweaks", "tool-segment-icon"));
  ...
  elements.toolSegments.replaceChildren(...segments);
}
