...
test("browser panel renders, streams, validates, and resets live tweaks", async () => {
  ...
  try {
    ...
    assert.equal(document.querySelector("#tool-segments").hidden, true);
    assert.equal(document.querySelector("#tool-segments").childElementCount, 0);
...
test("switches available tools beside the app-only picker", async () => {
  const document = makeDocument();
  ...
  try {
    await import(`./app.js?tool-segments-test=${Date.now()}`);
    await delay(0);

    const group = document.querySelector("#tool-segments");

    assert.equal(group.hidden, false);
    assert.equal(group.childElementCount, 2);
    assert.equal(document.querySelector("#app-name").textContent, app.name);
    assert.equal(findInput(group, "Network tool").getAttribute("aria-pressed"), "false");
    assert.equal(findInput(group, "Tweaks tool").getAttribute("aria-pressed"), "true");
    assert.equal(findInput(group, "Network tool").textContent, "");
    assert.equal(findInput(group, "Network tool").children[0].tagName, "SVG");
    assert.equal(findInput(group, "Tweaks tool").children[0].tagName, "SVG");
    assert.ok(findAppChoice(document, app, "Network"));
    assert.ok(findAppChoice(document, app, "Tweaks"));

    await findInput(group, "Network tool").emit("click");

    assert.equal(document.querySelector("#app-name").textContent, app.name);
    assert.equal(findInput(group, "Network tool").getAttribute("aria-pressed"), "true");
    assert.equal(findInput(group, "Tweaks tool").getAttribute("aria-pressed"), "false");
  } finally {
    globalThis.document = originalDocument;
    globalThis.fetch = originalFetch;
  }
