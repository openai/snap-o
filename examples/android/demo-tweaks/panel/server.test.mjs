...
test("serves the host-side tweaks panel", async () => {
  ...
  assert.match(html, /id="app-picker"[\s\S]*?id="tool-segments"/);
...
test("uses a responsive, monochrome tool dashboard", async () => {
  const response = await fetch(new URL("/styles.css", panel.url));
  const styles = await response.text();
...
test("serves Network Tool's official Lucide app-picker chevron", async () => {
