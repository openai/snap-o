...
```bash
./gradlew :samples:demo-okhttp:assembleDebug
adb -s <serial> install -r examples/android/<demo>/build/outputs/apk/debug/<demo>-debug.apk
...
From the repository root, run both modes without building release variants:
...
The existing network buttons remain available while the task routes are active. Inspect task requests with Snap-O's network tool or `network requests` and `network show`. The full route API is documented in the [Network Interception guide](../../docs/network-intercept.md).
