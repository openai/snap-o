...
import com.openai.snapo.discovery.InspectorResources.ProcessInfo;
import static com.openai.snapo.discovery.InspectorResources.*;
...
public final class FrontendMain {
    ...
    public static void main(String[] args) {
        ...
        try {
            if (args.length != 2 || args[1].length() > 16384) {
                throw new IllegalArgumentException("Pass an inspector socket and expected package metadata.");
            }
    ...
    private static byte[] readFrontend(Context system, int pid, String inspectorId, JSONObject expected) throws Exception {
        ...
        JSONObject inspector = readInspector(pm, app, resources, PREFIX + inspectorId);
        JSONObject frontend = inspector.getJSONObject("frontend");
        if (!process.identity.equals(expected.getString("processIdentity"))
                ...
                || !inspectorId.equals(expected.getString("inspectorId"))
            ...
            throw new IllegalStateException("Inspector frontend does not match the selected app.");
        }
        ...
        if (!process.identity.equals(new ProcessInfo(pid).identity)
            ...
            throw new IllegalStateException("App changed while reading inspector frontend.");
        }
        return archive;
    }
