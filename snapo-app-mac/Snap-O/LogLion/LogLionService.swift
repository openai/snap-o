...
actor LogLionService {

  private struct DeviceState {
    var streamTask: Task<Void, Never>?
    var continuations: [UUID: AsyncStream<LogLionEvent>.Continuation] = [:]
    var events: [LogLionEvent] = []
  ...
  deinit {
    ...
    for (deviceID, state) in deviceStates {
      ...
      logger.debug("Deinitializing LogLionService cancelled stream for \(deviceID, privacy: .public)")
    }
  }
  ...
  func eventsStream(for deviceID: String) -> AsyncStream<LogLionEvent> {
  ...
  private func registerContinuation(id: UUID,
                                    deviceID: String,
                                    continuation: AsyncStream<LogLionEvent>.Continuation) async {
    ensureDeviceState(for: deviceID)
    var state = deviceStates[deviceID]!
  ...
  private func runStream(for deviceID: String) async {
    ...
    while !Task.isCancelled {
      ...
      do {
        ...
        try socket.sendShell("logcat -v threadtime")
        ...
        while !Task.isCancelled {
          ...
          while let newline = buffer.firstIndex(of: UInt8(ascii: "\n")) {
            ...
            if let line = String(data: lineData, encoding: .utf8) {
              logger.debug("\(line)")
              let entry = LogLionLineParser.parseThreadtime(line)
        ...
        logger.error(
          "LogLion ADB stream error for \(deviceID, privacy: .public): \(lastErrorReason ?? "unknown", privacy: .public)"
        )
  ...
  private func recordEntry(_ entry: LogLionEntry, for deviceID: String) async {
  ...
  private func appendEvent(_ event: LogLionEvent, to deviceID: String) {
    guard var state = deviceStates[deviceID] else { return }
