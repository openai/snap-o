...
enum LogLionParserError: Error {
  case malformedLine(String)
}

struct LogLionLineParser {
  private static let threadtimePattern = #"^\s*(?<month>\d{2})-(?<day>\d{2})\s+(?<time>\d{2}:\d{2}:\d{2}\.\d{3})\s+(?<pid>\d+)\s+(?<tid>\d+)\s+(?<level>[A-Z])\s+(?<tag>.+?):\s+(?<message>.*)$"#
  ...
  static func parseThreadtime(_ line: String, calendar: Calendar = .current) -> LogLionEntry {
    ...
    let level = LogLionLevel(symbol: levelSymbol)
