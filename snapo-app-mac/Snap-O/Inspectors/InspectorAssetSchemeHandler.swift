...
@MainActor
final class InspectorAssetSchemeHandler: NSObject, WKURLSchemeHandler {
  static let scheme = "snapo-inspector"
  let baseURL: URL
  var bundle: InspectorFrontendBundle?

  init(storageIdentifier: UUID?) {
    guard let url = URL(string: "\(Self.scheme)://\((storageIdentifier ?? UUID()).uuidString.lowercased())/") else {
      preconditionFailure("Invalid inspector asset URL")
    }
    baseURL = url
    super.init()
  }
  ...
  func webView(_ webView: WKWebView, start urlSchemeTask: any WKURLSchemeTask) {
    guard let url = urlSchemeTask.request.url, url.scheme == Self.scheme, url.host == baseURL.host,
          ...
          let response = HTTPURLResponse(url: url, statusCode: 200, httpVersion: "HTTP/1.1", headerFields: [
            ...
            "Content-Security-Policy": InspectorWebPolicy.contentSecurityPolicy,
