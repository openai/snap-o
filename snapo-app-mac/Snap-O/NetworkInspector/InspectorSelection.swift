...
struct InspectorSelection {
  ...
  private var kind: AppInspectorKind?
  private var current: SelectedAppInspector?
  private var retained: [AppInspectorKind: SelectedAppInspector] = [:]
  ...
  var state: AppInspectorState {
    AppInspectorState(
      apps: apps,
      selection: current,
      displayedNetwork: retained[.network],
      displayedTweaks: retained[.tweaks],
      selectedApp: target,
      replacementApp: replacementApp,
      preferredKind: kind,
      isRestoring: awaitingAppIdentity || (current == nil && (kind != nil || apps.contains { !$0.inspectors.isEmpty }))
    )
  }
  ...
  mutating func reconcile(_ apps: [InspectableApp]) {
    ...
    if let app, let kind {
      if let target, app.id != target.id, !retained.isEmpty {
        ...
        setCurrent(app, option: app.inspectors.first { $0.kind == kind })
  ...
  mutating func selectInspector(_ app: InspectableApp, option: AppInspectorOption) {
  ...
  private var replacementApp: InspectableApp? {
    ...
    return apps.first {
      $0.id != target.id && Self.sameApp(target, $0) && $0.inspectors.contains { $0.kind == kind }
    }
  }

  private mutating func selectKind(_ app: InspectableApp, kind: AppInspectorKind) {
    startupSelectionPending = false
    ...
    setCurrent(liveApp ?? app, option: liveApp?.inspectors.first { $0.kind == kind })
  }

  private mutating func updateTarget(_ app: InspectableApp) {
    var options: [AppInspectorKind: AppInspectorOption] = [:]
    if let target, Self.sameApp(target, app) {
      for option in target.inspectors {
  ...
  private mutating func remember(_ app: InspectableApp, kind: AppInspectorKind) {
