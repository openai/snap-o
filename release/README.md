...
- Version against the last published contract. Keep its number when compatible, or increment it once for breaking changes in the next release. New contracts start at 1. Do not increment again for intermediate unreleased revisions or add compatibility fallbacks for them.
...
Confirm `tool-core` appears in the publication manifest and in the Network and Tweaks runtime dependencies. Its AAR must not contribute a tool descriptor, frontend ZIP, or initialization provider. Core API changes must pass the independent Example build and HTTP routing/streaming tests, including client disconnect cancellation.
