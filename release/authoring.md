...
This setup prepares the plugin SDK for future publication. Local validation does not publish packages.
...
| Host JavaScript SDK | npm | `sdk/host/package.json` |

The Gradle plugin configures both project/settings implementations and their marker publications. The marker lets a consumer use `id("com.openai.snapo.plugin") version "…"` with `mavenCentral()` in `pluginManagement.repositories`. A Plugin Portal release is optional and is not configured here.
...
```sh
./gradlew -p sdk/gradle-plugin assembleMavenCentralPublication
```
...
5. With explicit authorization, publish the SDK with `npm publish` from `sdk/host/`. Its `prepack` script builds the same files checked by local validation. Configure npm publishing authentication or trusted publishing for the final package at that time.
