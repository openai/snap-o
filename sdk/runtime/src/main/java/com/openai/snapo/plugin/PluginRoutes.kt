...
/** Routes are registered once, before the server starts. JSON serialization belongs to the plugin. */
class PluginRoutes internal constructor() {
    var requestPolicy = PluginHttpRequestPolicy()
    var preflightStatusCode = 204
    var exposedHeaders: String? = null
    var vary = "Origin"
    var cacheControl = "no-store"
    internal val entries = mutableListOf<PluginRoute>()
    internal var validate: (PluginHttpRequest) -> Unit = {}
    internal var errorResponse: (Exception) -> PluginHttpResponse = ::defaultErrorResponse
    internal var fallback: suspend PluginCall.() -> Unit = { throw PluginHttpException(404, "Unknown endpoint") }

    fun get(path: String, handler: suspend PluginCall.() -> Unit) = route("GET", path, handler)
    fun post(path: String, handler: suspend PluginCall.() -> Unit) = route("POST", path, handler)
    fun put(path: String, handler: suspend PluginCall.() -> Unit) = route("PUT", path, handler)
    fun patch(path: String, handler: suspend PluginCall.() -> Unit) = route("PATCH", path, handler)
    fun delete(path: String, handler: suspend PluginCall.() -> Unit) = route("DELETE", path, handler)

    /** Sends a heartbeat comment every 30 seconds by default. Null disables automatic heartbeats. */
    fun sse(path: String, heartbeatInterval: Duration? = 30.seconds, handler: suspend PluginSseSession.() -> Unit) {
    ...
    fun route(method: String, path: String, handler: suspend PluginCall.() -> Unit) {
        ...
        entries.add(PluginRoute(method, path, handler))
    }

    /** Optional protocol-specific validation, after HTTP/browser validation and before routing. */
    fun validateRequest(validate: (PluginHttpRequest) -> Unit) { this.validate = validate }

    /** Customize domain errors. Responses already started are never replaced with an error response. */
    fun onError(response: (Exception) -> PluginHttpResponse) { errorResponse = response }

    fun notFound(handler: suspend PluginCall.() -> Unit) { fallback = handler }
}

internal class PluginRoute(
    val method: String,
    val path: String,
    val handler: suspend PluginCall.() -> Unit,
) {
...
internal fun defaultErrorResponse(error: Exception): PluginHttpResponse {
    val status = when (error) {
        is PluginHttpException -> error.statusCode
        is java.net.SocketTimeoutException -> 408
        is IllegalArgumentException, is java.io.IOException -> 400
        else -> 500
    }
    val message = if (status == 500) "Internal server error" else error.message ?: "Invalid request"
    return PluginHttpResponse.error(status, message, (error as? PluginHttpException)?.allowedMethods)
}
