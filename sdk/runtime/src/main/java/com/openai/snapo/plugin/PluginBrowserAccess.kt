...
/** Browser access policy for plugin sockets forwarded to loopback. */
object PluginBrowserAccess {
    fun origin(headers: Map<String, String>): String? {
        ...
        val allowedAuthority = PluginOrigin.matches(value) ||
    ...
    /** Pass only an origin returned by [origin]. Methods and exposed headers belong to the plugin. */
    fun responseHeaders(
        origin: String?,
...
private val PluginOrigin = Regex("snapo-inspector://[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}")
