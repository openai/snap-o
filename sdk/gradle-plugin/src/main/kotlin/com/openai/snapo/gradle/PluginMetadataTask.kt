...
@CacheableTask
abstract class PluginMetadataTask : DefaultTask() {
    ...
    fun generate() {
        ...
        require(displayName.get().isNotBlank() && displayName.get().length <= 200) {
            "Plugin display name must contain between 1 and 200 characters"
        }
        require(protocolVersion.get() > 0) { "Plugin protocol version must be positive" }
        require(hostApiVersion.get() > 0) { "Plugin host API version must be positive" }
        val source = sourceDirectory.file("${namespace.get().replace('.', '/')}/SnapOPlugin.java").get().asFile
        sourceDirectory.get().asFile.deleteRecursively()
        source.parentFile.mkdirs()
        source.writeText("""
            package ${namespace.get()};

            /** Generated from snapoPlugin. */
            public final class SnapOPlugin {
