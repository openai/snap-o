...
describe("shared plugin host", () => {
  it("publishes metadata that arrives while the process remains disconnected", async () => {
    ...
    const plugin = { id: "sample", name: "Sample", protocolVersion: 4 };
    const manifest = { version: 1, pid: 10 };
    emit("connection", { revision: 3, connected: false, manifest, inspector: plugin });
    expect(changed).toHaveBeenCalledOnce();
    expect(host.connected).toBe(false);
    expect(host.manifest).toEqual(manifest);
    expect(host.plugin).toEqual(plugin);
  });
