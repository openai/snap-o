...
export function checkBoundaries(root, packages) {
  ...
  for (const name of packages) {
    ...
    if (name === "sdk/host" && Object.keys(deps).some((dep) => /^(preact|react|@snap-o\/)/.test(dep)))
      errors.push("sdk/host must not depend on UI or tool packages");
      ...
      function visit(node) {
        ...
        if (specifier && ts.isStringLiteralLike(specifier)) {
          ...
          if (resolved) {
            ...
            if (owner && owner !== name && !(owner === "sdk/host" && value === "@snap-o/plugin-host"))
              errors.push(`${file}: forbidden package import ${value}`);
            if (name === "sdk/host" && /^(preact|react)(\/|$)/.test(value))
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const errors = checkBoundaries(fileURLToPath(new URL("../../..", import.meta.url)), [
    "sdk/host",
    "plugins/network/frontend",
    "plugins/tweaks/frontend"
  ]);
  if (errors.length) {
