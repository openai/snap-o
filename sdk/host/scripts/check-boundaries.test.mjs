...
test("boundaries reject relative, alias, and dynamic imports across tools", async () => {
  ...
  try {
    const directories = ["network/frontend", "tweaks/frontend", "sdk/host"];
    ...
    await writeFile(path.join(root, "sdk/host/src/index.ts"), 'import "../../../network/frontend/src/index";');
    assert.equal(checkBoundaries(root, directories).length, 4);
    await writeFile(
      path.join(root, "sdk/host/package.json"),
      JSON.stringify({ name: "@snap-o/plugin-host", dependencies: { preact: "*" } })
    );
    assert.ok(checkBoundaries(root, directories).includes("sdk/host must not depend on UI or tool packages"));
  } finally {
