...
import com.openai.snapo.inspector.InspectorHttpException
...
internal class NetworkInspectorHttp(
    ...
    val server = InspectorServer(SnapOInspector.ID, maxConnections = 128) {
    ...
    private fun eventStreamRequested(request: InspectorHttpRequest): Boolean {
        ...
        if (preferences.all { it == 0.0 }) {
            throw InspectorHttpException(406, "Use application/x-ndjson or text/event-stream")
        }
        return preferences[1] > preferences[0]
    }

    private suspend fun InspectorCall.body(method: String) {
        ...
        val result = reply?.result ?: throw InspectorHttpException(404, reply?.error?.message ?: "Body is unavailable")
        respondJson(result.toString())
    }

    private suspend fun InspectorCall.stream(routes: JsonObject?) {
        if (!streamSlots.tryAcquire()) throw InspectorHttpException(503, "Too many event streams")
    ...
    private fun runner(id: String): NetworkEventStream = runners[id]?.takeUnless { it.isClosed }
        ?: throw InspectorHttpException(410, "Interception runner has ended")
    ...
    private fun withRunner(stream: NetworkEventStream, errorStatus: Int, action: () -> Unit) {
        try {
            ...
            throw InspectorHttpException(errorStatus, error.message ?: "Invalid interception request", error)
        }
    ...
    private suspend fun InspectorCall.history() {
    ...
    private fun notFound(): Nothing = throw InspectorHttpException(404, "Unknown endpoint")

    private fun requireMethod(request: InspectorHttpRequest, method: String) {
        if (request.method != method) throw InspectorHttpException(405, "Use $method")
    }
