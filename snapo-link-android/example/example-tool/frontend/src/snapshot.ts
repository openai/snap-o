...
export function observeExample(
  ...
  function available() {
    return (
      ...
      connection.baseURL &&
      connection.inspector?.protocolVersion === 1
    );
  }
  ...
  function connectionChanged() {
    ...
    if (!connected) return;
    if (connection.inspector?.protocolVersion !== 1) {
  ...
  connection.addEventListener("connection", connectionChanged);
  connectionChanged();
  return {
    refresh,
    increment,
    dispose() {
      stopped = true;
      disconnect();
      connection.removeEventListener("connection", connectionChanged);
    },
  };
}
