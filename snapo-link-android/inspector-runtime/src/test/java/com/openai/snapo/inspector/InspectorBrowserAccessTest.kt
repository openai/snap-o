...
class InspectorBrowserAccessTest {
    @Test
    fun `HTTP rejects rebinding hosts even when the browser omits Origin`() {
        for (host in listOf("attacker.example:1234", "localhost.attacker.example", "127.0.0.1.attacker.example")) {
            for (origin in listOf(null, "http://$host", "http://localhost")) {
                ...
                assertThrows(IllegalArgumentException::class.java) { InspectorBrowserAccess.origin(headers) }
            }
        }
    }
    ...
    fun `HTTP accepts native clients and loopback browser origins`() {
        for (host in listOf("localhost", "127.0.0.1:1234", "[::1]:1234")) {
            assertNull(InspectorBrowserAccess.origin(mapOf("host" to host)))
            for (origin in listOf(
                ...
                assertEquals(origin, InspectorBrowserAccess.origin(mapOf("host" to host, "origin" to origin)))
            }
        }
    }
    ...
    fun `HTTP rejects missing or malformed hosts and remote or opaque origins`() {
        assertThrows(IllegalArgumentException::class.java) { InspectorBrowserAccess.origin(emptyMap()) }
        for (host in listOf("", "user@localhost", "localhost/path", "localhost?query", "localhost#fragment")) {
            assertThrows(IllegalArgumentException::class.java) { InspectorBrowserAccess.origin(mapOf("host" to host)) }
        }
        for (origin in listOf(
            ...
            assertThrows(IllegalArgumentException::class.java) {
                InspectorBrowserAccess.origin(mapOf("host" to "localhost", "origin" to origin))
            }
        }
    }
}
