...
class InspectorHttpTest {
    ...
    fun `PATCH supports UTF-8 bodies and configurable content type and protocol policies`() {
        ...
        val policy = InspectorHttpRequestPolicy(
            maxBodyBytes = body.size,
            httpVersions = setOf("HTTP/1.0", "HTTP/1.1"),
            requireJsonContentType = false,
        )
        val request = InspectorHttpRequest.read(ByteArrayInputStream(head.toByteArray() + body), policy)
        assertEquals("PATCH", request.method)
        assertTrue(body.contentEquals(request.body))
        assertThrows(IllegalArgumentException::class.java) {
            InspectorHttpRequest.read(ByteArrayInputStream(head.toByteArray() + body))
        }
    }

    @Test
    fun `oversized bodies and invalid origins are rejected before consuming body bytes`() {
        val policy = InspectorHttpRequestPolicy(maxBodyBytes = 3)
        val head = "POST / HTTP/1.1\r\nHost: localhost\r\nContent-Length: 4\r\n" +
            "Content-Type: application/json\r\n\r\n"
        val input = ByteArrayInputStream((head + "test").toByteArray())
        assertEquals(
            413,
            assertThrows(InspectorHttpException::class.java) {
                InspectorHttpRequest.read(input, policy)
            }.statusCode
        )
        assertEquals(4, input.available())
        val remote = ByteArrayInputStream((head.replace("localhost", "example.test") + "test").toByteArray())
        assertThrows(IllegalArgumentException::class.java) { InspectorHttpRequest.read(remote) }
    ...
    fun `responses include byte lengths and inspector specific preflight headers`() {
        ...
        InspectorHttpResponse(405, body, allowedMethods = "GET, PATCH").write(
            output,
            InspectorBrowserAccess.responseHeaders("http://localhost:5173", "GET, PATCH"),
        )
        ...
        assertThrows(IllegalArgumentException::class.java) {
            InspectorHttpResponse(200, byteArrayOf()).write(output, mapOf("Location" to "/\r\nInjected: yes"))
        }
    }
    ...
    fun `SSE frames multiline data without allowing field injection`() {
        assertEquals(
            "event: update\nid: 7\ndata: first\ndata: second\n\n",
            InspectorSse.event("first\r\nsecond", "update", "7").toString(Charsets.UTF_8),
        )
        assertThrows(IllegalArgumentException::class.java) { InspectorSse.event("{}", "update\ndata: injected") }
        assertThrows(IllegalArgumentException::class.java) { InspectorSse.event("{}", id = "7\u0000") }
    }

    private fun parse(request: String) = InspectorHttpRequest.read(ByteArrayInputStream(request.toByteArray()))
}
