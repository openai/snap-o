...
class InspectorSocketServerTest {
    @Test
    fun `start is idempotent and close releases a blocked reader and listener`() {
        ...
        val server = InspectorSocketServer(1, {
    ...
    fun `connection limits reject excess clients without disturbing an active request`() {
        val listener = TcpListener()
        val entered = CountDownLatch(1)
        InspectorSocketServer(1, { listener }) { connection ->
            entered.countDown()
    ...
    fun `socket names follow the inspector discovery contract`() {
        assertEquals("snapo_example.inspector-v2_123", InspectorSocketServer.socketName("example.inspector-v2", 123))
        for (id in listOf("", "Uppercase", "with_underscore", "a".repeat(101), "../example")) {
            assertThrows(IllegalArgumentException::class.java) { InspectorSocketServer.socketName(id, 123) }
        }
        assertThrows(IllegalArgumentException::class.java) { InspectorSocketServer.socketName("example", 0) }
    }

    private class TcpListener : InspectorSocketListener {
        ...
        override fun accept(): InspectorConnection = TcpConnection(socket.accept())
    }

    private class TcpConnection(private val socket: Socket) : InspectorConnection {
