...
class InspectorRoutes internal constructor() {
    var requestPolicy = InspectorHttpRequestPolicy()
    var preflightStatusCode = 204
    var exposedHeaders: String? = null
    var vary = "Origin"
    var cacheControl = "no-store"
    internal val entries = mutableListOf<InspectorRoute>()
    internal var validate: (InspectorHttpRequest) -> Unit = {}
    internal var errorResponse: (Exception) -> InspectorHttpResponse = ::defaultErrorResponse
    internal var fallback: suspend InspectorCall.() -> Unit = { throw InspectorHttpException(404, "Unknown endpoint") }

    fun get(path: String, handler: suspend InspectorCall.() -> Unit) = route("GET", path, handler)
    fun post(path: String, handler: suspend InspectorCall.() -> Unit) = route("POST", path, handler)
    fun put(path: String, handler: suspend InspectorCall.() -> Unit) = route("PUT", path, handler)
    ...
    fun route(method: String, path: String, handler: suspend InspectorCall.() -> Unit) {
        ...
        entries.add(InspectorRoute(method, path, handler))
    }

    /** Optional protocol-specific validation, after HTTP/browser validation and before routing. */
    fun validateRequest(validate: (InspectorHttpRequest) -> Unit) { this.validate = validate }

    /** Customize domain errors. Responses already started are never replaced with an error response. */
    fun onError(response: (Exception) -> InspectorHttpResponse) { errorResponse = response }

    fun notFound(handler: suspend InspectorCall.() -> Unit) { fallback = handler }
}

internal class InspectorRoute(
    val method: String,
    val path: String,
    val handler: suspend InspectorCall.() -> Unit,
) {
...
internal fun defaultErrorResponse(error: Exception): InspectorHttpResponse {
    val status = when (error) {
        is InspectorHttpException -> error.statusCode
    ...
    return InspectorHttpResponse.error(status, message, (error as? InspectorHttpException)?.allowedMethods)
}
