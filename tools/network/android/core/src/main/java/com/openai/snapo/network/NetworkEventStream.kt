...
import com.openai.snapo.tool.ToolSse
...
internal class NetworkEventStream {
    ...
    @Volatile
    private var session: ToolSseSession? = null
    ...
    suspend fun serve(session: ToolSseSession) {
...
internal fun sseEvent(text: String, event: String? = null, sequence: Long? = null): ByteArray {
    ...
    return ToolSse.event(text, event, sequence?.toString())
}
