...
import { applyRequestBodies, type ToolRecord, type RequestRecord } from "../../../network/cdp";
...
function harBodyTextBytes(record: ToolRecord): number {
...
function omittingHarBodyText(record: ToolRecord): ToolRecord {
  if (record.kind === "websocket") {
    return {
      ...record,
