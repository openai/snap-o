...
export function useHostConnection(host: Host) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = host.onConnection((connection) => {
      ...
      void fetch(new URL("network/protocol", connection.baseURL), { signal })
        .then(async (response) => {
          if (!response.ok)
          ...
          if (
            ...
            throw new Error("The Android app returned an invalid Network protocol version.");
          }
        ...
        .catch((error: unknown) => {
          ...
          setState((current) => ({
            ...current,
            connected: false,
            error: error instanceof Error ? error.message : "Unable to check the Network protocol."
          }));
        });
    });
    return () => {
