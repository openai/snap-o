...
export type ToolRecord = RequestRecord | WebSocketRecord;

export const toolRetentionLimits = {
  records: 2_000,
  streamEventsPerRequest: 1_000,
  webSocketMessagesPerSocket: 2_000
} as const;

export function createEmptyToolState(): ToolDataState {
  return {
    requests: new Map(),
    webSockets: new Map(),
...
export function reduceCdpMessage(
  ...
  return enforceToolRetention(reduced);
}
...
function reduceEventSourceMessage(
  ...
  return updateRequest(state, processId, requestId(params), (existing) => {
    ...
    return {
      ...base,
      status: { kind: "pending" },
      streamEvents: retainNewest([...base.streamEvents, nextEvent], toolRetentionLimits.streamEventsPerRequest),
...
export function recordId(record: ToolRecord): string {
...
function acceptSequence(state: ToolDataState, processId: string, sequence: number | undefined): ToolDataState | null {
...
export function enforceToolRetention(
  state: ToolDataState,
  maxRecords: number = toolRetentionLimits.records
): ToolDataState {
  ...
  return updateWebSocket(state, processId, requestId(params), (existing) => {
    ...
    return {
      ...
      messages: retainNewest(
        ...
        toolRetentionLimits.webSocketMessagesPerSocket
      ),
