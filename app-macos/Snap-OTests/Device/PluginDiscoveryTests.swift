...
struct PluginDiscoveryTests {
  ...
  func parsesSharedSnapshot() {
    ...
    """
    let sockets = PluginDiscovery.sockets(inProcNetUnix: output, deviceID: "phone")
    #expect(sockets.map(\.kind) == [.network, .tweaks, PluginID(rawValue: "unknown")])
  ...
  func validatesSocketNames() {
    ...
    for name in [
      ...
      #expect(PluginDiscovery.sockets(inProcNetUnix: output, deviceID: "phone").isEmpty)
    }
  }
  ...
  func preservesBridgeEncoding() throws {
    ...
    #expect(try String(bytes: encoder.encode(PluginID.tweaks), encoding: .utf8) == "\"tweaks\"")
    let server = PluginServerReference(deviceId: "device", socketName: "snapo_tweaks_42")
  ...
  func ignoresClientSockets() throws {
    ...
    for inode in ["0", "202", "303"] {
      ...
      for snapshot in [clients + "\n" + listener, listener + "\n" + clients] {
        let sockets = PluginDiscovery.sockets(inProcNetUnix: snapshot, deviceID: "phone")
        #expect(sockets.count == 1)
        #expect(try #require(sockets.first).inode == "101")
      }
      #expect(PluginDiscovery.sockets(inProcNetUnix: clients, deviceID: "phone").isEmpty)
    }
  }
  ...
  func includesInitialProcessNames() throws {
    ...
    let process = try #require(PluginDiscovery.processes(from: [
      endpoint(socket.kind, metadata: PluginAppMetadata(processName: socket.processName))
    ]).first)
  ...
  func cleansUpManifestReader() throws {
    ...
    let command = try PluginManifestReader.command(helper: helper, socketNames: ["snapo_network_42"])
  ...
  func requiresProcessIdentity() throws {
    ...
    for identity: Any in [NSNull(), "", " ", 42] {
      ...
      #expect(throws: (any Error).self) { try PluginManifestReader.decode(data) }
    }
    let success = try JSONSerialization.data(withJSONObject: [
      "version": 1, "pid": 42, "app": app, "processIdentity": "boot:42:1"
    ])
    #expect(try PluginManifestReader.decode(success).first?.processIdentity == "boot:42:1")
    let failure = Data(#"{"version":1,"pid":42,"error":"process exited"}"#.utf8)
    #expect(try PluginManifestReader.decode(failure).first?.error == "process exited")
  }

  @Test("merges tool sockets before any app info is available")
  func mergesWithoutMetadata() throws {
    let processes = PluginDiscovery.processes(from: [endpoint(.tweaks), endpoint(.network)])
  ...
  func improvesMetadata() throws {
    let initial = try #require(PluginDiscovery.processes(from: [endpoint(.network)]).first)
    let network = endpoint(.network, metadata: PluginAppMetadata(
      processName: "com.example.demo:worker",
      packageName: "com.example.demo",
      packageNameHint: "com.example.demo:worker",
      appIconBase64: "network-icon"
    ))
    let tweaks = endpoint(.tweaks, metadata: PluginAppMetadata(
      appName: "Demo App",
      packageName: "com.example.demo",
      androidUserID: 10
    ))
    let loaded = try #require(PluginDiscovery.processes(from: [tweaks, network]).first)
  ...
  func separatesProcesses() {
    let metadata = PluginAppMetadata(packageName: "com.example.demo")
    let processes = PluginDiscovery.processes(from: [
  ...
  func usesBestNonemptyMetadata() throws {
    ...
    let legacy = try #require(PluginDiscovery.processes(from: [network]).first)
    #expect(legacy.metadata.packageName == nil)
    let tweaks = endpoint(.tweaks, metadata: PluginAppMetadata(
      appName: " ", packageName: "com.example.demo", appIconBase64: "tweaks-icon"
    ))
    let process = try #require(PluginDiscovery.processes(from: [network, tweaks]).first)
  ...
  func keepsUnknownSocketsSeparate() {
    let endpoints = ["legacy-one", "legacy-two"].map {
      PluginEndpoint(
        kind: .network,
        reference: PluginServerReference(deviceId: "device", socketName: $0),
        deviceDisplayTitle: "Device",
        metadata: PluginAppMetadata(appName: "Same name")
      )
    }
    #expect(PluginDiscovery.processes(from: endpoints).map(\.id) == [
  ...
  private func endpoint(
    ...
    pid: Int = 42,
    metadata: PluginAppMetadata = PluginAppMetadata()
  ) -> PluginEndpoint {
    PluginEndpoint(
      kind: kind,
      reference: PluginServerReference(deviceId: device, socketName: "snapo_\(kind.rawValue)_\(pid)"),
      deviceDisplayTitle: "Device",
      pid: pid,
      metadata: metadata
    )
...
extension PluginDiscoveryTests {
  ...
  func legacyMetadataEvidence() throws {
    ...
    #expect(LegacyPluginReader.requests(kind: PluginID(rawValue: "custom")).isEmpty)
