...
struct PluginFrontendTests {
  ...
  func loadsAssets() throws {
    ...
    let bundle = try PluginFrontendBundle(archive: archive(files))
  ...
  func rejectsUnsafeEntries() throws {
    ...
    for path in ["../escape.js", "/absolute.js", "assets/../../escape", "assets\\escape", "assets//file", "assets/./file", "index.html"] {
      ...
      #expect(throws: (any Error).self) { try PluginFrontendBundle(archive: data) }
    }
    let link = try archive([("index.html", Data("/tmp/other".utf8))], type: .symlink)
    #expect(throws: (any Error).self) { try PluginFrontendBundle(archive: link) }
  }
  ...
  func boundsExpansion() throws {
    ...
    #expect(throws: (any Error).self) { try PluginFrontendBundle(archive: oversized) }
    let many = try archive((0 ..< 1025).map { ("file\($0)", Data()) })
    #expect(throws: (any Error).self) { try PluginFrontendBundle(archive: many) }
    #expect(throws: (any Error).self) { try PluginFrontendBundle(archive: Data("not a ZIP".utf8)) }
  }

  @Test("frontend requests are tied to the discovered package and tool")
  func requestsExpectedPackage() throws {
    let manifest = try JSONDecoder().decode(
      PluginProcessMetadata.self,
      from: Data(
    ...
    let request = try PluginFrontendBundle.request(manifest: manifest, tool: tool)
    ...
    let identity = try #require(PluginProcessIdentity(metadata: manifest))
    let normalizedRequest = try PluginFrontendBundle.request(identity: identity, tool: tool)
    ...
    for (key, invalid) in [("androidUserId", NSNull()), ("androidUserId", -1), ("processIdentity", " "), ("pid", 0)] as [(String, Any)] {
      var value = original
      value[key] = invalid
      let incomplete = try JSONDecoder().decode(PluginProcessMetadata.self, from: JSONSerialization.data(withJSONObject: value))
      #expect(PluginProcessIdentity(metadata: incomplete) == nil)
      #expect(throws: (any Error).self) { try PluginFrontendBundle.request(manifest: incomplete, tool: tool) }
    }
  }
}
