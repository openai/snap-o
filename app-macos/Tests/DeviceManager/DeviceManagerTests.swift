...
struct DeviceManagerTests {
  static func main() {
    ...
    print("Device Manager list tests passed (5 tests)")
  }
  ...
  private static func unmatchedDevicesRemainVisible() {
    ...
    let entries = DeviceManagerEntry.list(emulators: [emulator(serial: nil)], connectedDevices: devices)
  ...
  private static func additionalEmulatorInstancesRemainVisible() {
    ...
    for serial: String? in [nil, "emulator-5554", "emulator-5556"] {
      let entries = DeviceManagerEntry.list(emulators: [emulator(serial: serial)], connectedDevices: devices)
