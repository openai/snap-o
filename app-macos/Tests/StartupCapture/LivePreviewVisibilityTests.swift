...
struct LivePreviewVisibilityTests {
  ...
  static func main() {
    ...
    for (aspectRatio, expectedSize) in sizingCases {
      ...
      view.layoutSubtreeIfNeeded()
      pump()
      precondition(host.starts == 1 && host.stops.isEmpty, "Tool toggles must not restart the stream")
      precondition(LivePreviewRendererView.displayView === displayView, "Tool toggles must preserve the display view")
