...
struct ToolWebViewTests {
  ...
  static func main() async throws {
    ...
    try await eventually("Network should load after discovery") {
      model.toolApps.count == 2 && model.isPageReady && model.webContainer?.webView.superview != nil
    }
    ...
    model.selectTool(first, option: first.tools.first { $0.kind == .sample }!)
    ...
    let marker = try await sample.evaluateJavaScript("document.querySelector('#sample-tool').textContent") as? String
    precondition(marker == "Sample tool", "Load the plugin HTML without assuming a root element")
    ...
    model.selectTool(first, option: first.tools.first { $0.kind == .tweaks }!)
    ...
    precondition(isolated is NSNull, "Each tool has separate storage")
    ...
    precondition(networkState == "network state", "Keep renderer state when switching tool types")
    ...
    print("Tool types reuse their own view and receive inactive connection state")

    let tweaksOnly = app(30, kinds: [.tweaks])
    await service.setApps([first, second, tweaksOnly])
    try await eventually("Discover an app that only provides Tweaks") { model.toolApps.count == 3 }
    ...
    try await eventually("Cached metadata should remain visible while the selected tool is disconnected") {
      model.isWaiting && model.toolApps.count == 2 && model.selectedToolApp?.id == first.id
    }
    precondition(model.webContainer?.webView === network && model.selectedToolApp?.tools.count == 3)
    ...
    print("Disconnected tool metadata keeps its row and page without retaining a live connection")
    ...
    try await eventually("Page recovery should reload the tool") { model.isPageReady }
    ...
    precondition(replacement !== network, "Changing the selected app replaces that tool's view")
    ...
    precondition(stored is NSNull, "Different providing apps cannot share tool storage")
    ...
    model.selectTool(second, option: second.tools.first { $0.kind == .tweaks }!)
    ...
    print("Changing apps replaces cached pages and isolates tool storage")

    model.selectApp(first)
    model.selectApp(second)
    model.selectApp(first)
    try await eventually("Rapid selection should load only the final page") {
      model.isPageReady && model.selectedToolApp?.id == first.id && model.webContainer?.webView.superview != nil
    }
    ...
    try await testUnsupportedTools(preferences: preferences)
  }
  ...
  static func testUnsupportedTools(preferences: UserDefaults) async throws {
    for compatibility: PluginCompatibility in [
      ...
      try await eventually("Unsupported tools should show their native explanation") {
      ...
      model.selectTool(provider, option: provider.tools[1])
    ...
    print("Unsupported and unreachable tools show native explanations without authorizing a frontend; compatible siblings still load")
  }
  ...
  static func testFrontendSources(preferences: UserDefaults) async throws {
    ...
    precondition(web.url?.scheme == ToolAssetSchemeHandler.scheme && web.url?.path == "/index.html")
    ...
    precondition(document["baseURI"] as? String == web.url?.absoluteString, "CSP must reject a tool-provided base URL")
    for invalid in ["", "https://example.com/", "file:///tmp/index.html", "http://user@localhost:1234/", "http://localhost:0/"] {
      precondition(ToolWebPolicy.developmentURL(invalid) == nil)
    }
    ...
    try await eventually("APK frontend should start without an override") {
      model.isPageReady && model.selectedToolApp?.id == first.id
    }
    precondition(model.developmentURL == nil)
    model.useDevelopmentServer(devURL)
    let preferenceKey = "inspectorDevelopmentServer." + ToolWebPolicy.storageIdentifier(app: first, tool: .tweaks)!.uuidString
  ...
  static func testSecurity() async throws {
    ...
    let fixture = try String(contentsOfFile: "Tests/ToolSelection/Fixtures/hostile.html", encoding: .utf8)
    ...
    precondition(rejected == true, "Hidden tools and invalid messages must not invoke native actions")
    ...
    let first = app(10)
    let scope = ToolWebPolicy.storageIdentifier(app: first, tool: .sample)
    precondition(scope != nil && scope != ToolWebPolicy.storageIdentifier(app: app(20), tool: .sample))
    precondition(scope != ToolWebPolicy.storageIdentifier(app: first, tool: .network))
    var restarted = app(11)
    restarted.metadata = first.metadata
    precondition(
      scope == ToolWebPolicy.storageIdentifier(app: restarted, tool: .sample),
      "Process IDs do not partition a provider's preferences"
    )
    restarted.metadata = nil
    precondition(ToolWebPolicy.storageIdentifier(app: restarted, tool: .sample) == nil, "Unknown providers use ephemeral storage")
    var nested: Any = "value"
    for _ in 0 ..< 10 {
      nested = ["nested": nested]
    }
    precondition(!ToolWebBridge.validMessage(["command": "setToolbar", "payload": nested], command: "setToolbar"))
    for invalid in ["https://127.0.0.1:1234/", "http://localhost:1234/", "http://127.0.0.1:1234/path", "http://127.0.0.1:1234/?q=1"] {
      precondition(!ToolWebPolicy.isPluginEndpoint(URL(string: invalid)!))
    }
    let oldURL = web.url
