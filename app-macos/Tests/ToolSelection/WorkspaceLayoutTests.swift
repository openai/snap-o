...
enum WorkspaceLayoutTests {
  static func run() throws {
    ...
    precondition(restored.layout == .tool && restored.capturePaneWidth == 420)
    ...
    restored.toggleTool()
    precondition(restored.layout == .tool, "Keep at least one pane visible")
    ...
    precondition(!mixed.showsTool, "Prefer the current key even when its value is false")
  ...
  private static func migration(defaults: UserDefaults, suite: String) {
    for visible in [false, true] {
      ...
      precondition(snapshot.showsTool == visible)
