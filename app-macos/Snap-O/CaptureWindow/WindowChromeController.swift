...
/// Makes the AppKit window chrome transparent while retaining the real grouped
...
private final class WindowTitleOverlayView: NSView {
  ...
  override func draw(_ dirtyRect: NSRect) {
    ...
    let centerX: CGFloat
    if let toolLeadingX {
      let toolLeft = overlayX(forContentX: toolLeadingX, contentView: contentView)
      leadingX = toolLeft + Self.horizontalPadding
      centerX = (toolLeft + contentRight) / 2
    } else {
