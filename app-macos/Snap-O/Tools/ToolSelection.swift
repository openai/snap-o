...
struct ToolSelection {
  ...
  mutating func reconcile(_ apps: [InspectableApp]) {
    ...
    if let app, let kind {
      if target != nil, !Self.sameProcess(target, app), !retained.isEmpty {
  ...
  mutating func selectApp(_ app: InspectableApp) {
    ...
    let savedKind = saved.apps.first { $0.matches(app) }?.kind
    let preferredKind = app.tools.first { $0.kind == savedKind }?.kind
  ...
  private var replacementApp: InspectableApp? {
    guard current == nil, let target, Self.hasIdentity(target) else { return nil }
    return apps.first {
      !Self.sameProcess(target, $0) && Self.sameApp(target, $0) && !$0.tools.isEmpty
    }
  }

  private mutating func selectKind(_ app: InspectableApp, kind: ToolID) {
    startupSelectionPending = false
    if !Self.sameProcess(target, app) { retained = [:] }
    awaitingAppIdentity = false
    let liveApp = apps.first { $0.id == app.id && Self.sameApp(app, $0) }
    updateTarget(liveApp ?? app)
    self.kind = kind
    saved.last = nil
    remember(app, kind: kind)
  ...
  private mutating func updateTarget(_ app: InspectableApp) {
    ...
    // Keep tool choices through rediscovery, but never carry them into another process.
    if let target, Self.sameProcess(target, app) {
  ...
  private static func sameProcess(_ first: InspectableApp?, _ second: InspectableApp) -> Bool {
    guard let first, first.id == second.id, sameApp(first, second) else { return false }
    if let firstIdentity = first.metadata?.verifiedIdentity?.processIdentity,
       let secondIdentity = second.metadata?.verifiedIdentity?.processIdentity {
