...
actor PluginService {
  ...
  private let httpService: PluginHTTPService
  private var toolAppOrder: [String: Int] = [:]
  private var refreshTask: Task<Void, Never>?
  private var isStopped = false
  private var frontends: [(key: [String], bundle: PluginFrontendBundle)] = []

  init(adbService: ADBService, deviceTracker: DeviceTracker) {
    self.adbService = adbService
    self.deviceTracker = deviceTracker
    httpService = PluginHTTPService(adbService: adbService)
  }
  ...
  func currentPlugins() async -> PluginDiscoverySnapshot {
    ...
    let appsByServer = Dictionary(uniqueKeysWithValues: applications.map {
      (PluginServerReference(deviceId: $0.deviceID, socketName: $0.socketName), $0)
    })
    let endpoints = applications.map { app in
      PluginEndpoint(
        kind: app.kind,
        reference: PluginServerReference(deviceId: app.deviceID, socketName: app.socketName),
        deviceDisplayTitle: app.deviceDisplayTitle,
        pid: app.pid,
        protocolVersion: app.protocolVersion,
        metadata: PluginAppMetadata(
          appName: app.name,
    ...
    let apps = PluginDiscovery.processes(from: endpoints).map { process in
      let candidates = process.tools.compactMap { appsByServer[$0.reference]?.metadata.process }
      var metadata = candidates.first { $0.verifiedIdentity != nil } ?? candidates.first ?? PluginMetadata.Process()
    ...
    return PluginDiscoverySnapshot(apps: orderedToolApps(apps), revision: snapshot.revision)
  }
  ...
  func pluginFrontend(
    ...
    guard let frontend = tool.frontend,
          frontend.hostApiVersion == 1 else { throw PluginError.frontendUnavailable }
    let key = [
      reference.deviceId,
  ...
  private func refreshNow() async {
    ...
    let adb = await adbService.exec()
    let sockets = await PluginDiscovery.discover(on: devices.map(\.id), using: adb)
