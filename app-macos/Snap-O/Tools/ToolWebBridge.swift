...
@MainActor
final class ToolWebBridge: NSObject, WKScriptMessageHandlerWithReply, NSWindowDelegate {
  ...
  private weak static var colorPanelOwner: ToolWebBridge?
  ...
  private func saveFile(_ input: ToolSaveFileInput) async throws -> ToolSaveFileResult {
    ...
    guard presentationWindow != nil, response == .OK, let url = panel.url else {
      return ToolSaveFileResult(saved: false, path: nil)
    }
    try data.write(to: url, options: .atomic)
    if isHAR {
      SaveLocation.setLastHARExportDirectoryURL(url.deletingLastPathComponent())
    }
    return ToolSaveFileResult(saved: true, path: nil)
  }
