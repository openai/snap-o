...
public struct PluginID: RawRepresentable, Hashable, Codable, Sendable {
...
public struct PluginAppMetadata: Sendable, Equatable {
...
public struct PluginEndpoint: Sendable {
  public let kind: PluginID
  public let pid: Int?
  public let reference: PluginServerReference
  public let deviceDisplayTitle: String
  public let protocolVersion: Int?
  public let metadata: PluginAppMetadata
...
public struct InspectableProcess: Sendable {
  ...
  public let deviceDisplayTitle: String
  public let metadata: PluginAppMetadata
  public let tools: [PluginEndpoint]

  public var name: String {
    metadata.appName ?? metadata.processName ?? metadata.packageName ?? metadata.packageNameHint
      ?? pid.map { "Process \($0)" } ?? tools[0].reference.socketName
  }
}

public struct DiscoveredPluginSocket: Sendable, Equatable {
  public let kind: PluginID
  public let pid: Int
  public let reference: PluginServerReference
...
public enum PluginDiscovery {
  ...
  public static func sockets(
    ...
    return sections[0].split(separator: "\n").compactMap { line in
      ...
      return DiscoveredPluginSocket(
        kind: PluginID(rawValue: id),
        pid: pid,
        reference: PluginServerReference(deviceId: deviceID, socketName: name),
        inode: String(inode),
  ...
  public static func processes(from endpoints: [PluginEndpoint]) -> [InspectableProcess] {
    Dictionary(grouping: endpoints, by: \.processID).map { id, endpoints in
      ...
      let first = ordered[0]
      let metadata = ordered.reduce(PluginAppMetadata()) { $0.merging($1.metadata) }
