...
public struct ToolAppMetadata: Sendable, Equatable {
...
public struct ToolEndpoint: Sendable {
  public let kind: ToolID
  public let pid: Int?
...
public enum ToolDiscovery {
  ...
  public static func sockets(
    ...
    return sections[0].split(separator: "\n").compactMap { line in
      ...
      return DiscoveredPluginSocket(
        kind: ToolID(rawValue: id),
        pid: pid,
        reference: ToolServerReference(deviceId: deviceID, socketName: name),
        inode: String(inode),
  ...
  public static func processes(from endpoints: [ToolEndpoint]) -> [InspectableProcess] {
    Dictionary(grouping: endpoints, by: \.processID).map { id, endpoints in
      ...
      let first = ordered[0]
      let metadata = ordered.reduce(ToolAppMetadata()) { $0.merging($1.metadata) }
