...
import com.openai.snapo.plugin.PluginHttpException
...
internal class NetworkToolHttp(
    ...
    val server = PluginServer(SnapOPlugin.ID, maxConnections = 128) {
    ...
    private fun eventStreamRequested(request: PluginHttpRequest): Boolean {
        ...
        if (preferences.all { it == 0.0 }) {
            throw PluginHttpException(406, "Use application/x-ndjson or text/event-stream")
        }
        return preferences[1] > preferences[0]
    }

    private suspend fun PluginCall.body(method: String) {
        ...
        val result = reply?.result ?: throw PluginHttpException(404, reply?.error?.message ?: "Body is unavailable")
        respondJson(result.toString())
    }

    private suspend fun PluginCall.stream(routes: JsonObject?) {
        if (!streamSlots.tryAcquire()) throw PluginHttpException(503, "Too many event streams")
    ...
    private fun runner(id: String): NetworkEventStream = runners[id]?.takeUnless { it.isClosed }
        ?: throw PluginHttpException(410, "Interception runner has ended")
    ...
    private fun withRunner(stream: NetworkEventStream, errorStatus: Int, action: () -> Unit) {
        try {
            ...
            throw PluginHttpException(errorStatus, error.message ?: "Invalid interception request", error)
        }
    ...
    private suspend fun PluginCall.history() {
    ...
    private fun notFound(): Nothing = throw PluginHttpException(404, "Unknown endpoint")

    private fun requireMethod(request: PluginHttpRequest, method: String) {
        if (request.method != method) throw PluginHttpException(405, "Use $method")
    }

    private fun PluginHttpRequest.json(): JsonObject =
