...
import type { ToolRecord, RequestStatus } from "../../../network/cdp";
import { statusDisplayName, statusToneClass } from "../lib/format";
import { recordShowsActiveIndicator } from "../lib/records";

export function StatusView({ record }: { record: ToolRecord }): JSX.Element {
...
export function StatusBadge({ record }: { record: ToolRecord }): JSX.Element | null {
  const status = record.status;
