...
function webKitMessageHandler(): WebKitMessageHandler | null {
  const hostWindow = window as Window & {
    webkit?: { messageHandlers?: { snapoHost?: WebKitMessageHandler } };
  };
  return hostWindow.webkit?.messageHandlers?.snapoHost ?? null;
}
...
export function requireNativeBridge(): WebKitMessageHandler {
  ...
  if (handler == null) throw new Error("Open this tool in the Snap-O macOS app.");
  return handler;
}
