...
test("boundaries reject relative, alias, and dynamic imports across tools", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "snapo-boundaries-"));
  try {
    const directories = ["network/frontend", "tweaks/frontend", "tool-sdk/host"];
    ...
    await writeFile(path.join(root, "tool-sdk/host/src/index.ts"), 'import "../../../network/frontend/src/index";');
    assert.equal(checkBoundaries(root, directories).length, 4);
    await writeFile(
      path.join(root, "tool-sdk/host/package.json"),
      JSON.stringify({ name: "@snap-o/tool-host", dependencies: { preact: "*" } })
    );
    assert.ok(checkBoundaries(root, directories).includes("tool-sdk/host must not depend on UI or tool packages"));
  } finally {
