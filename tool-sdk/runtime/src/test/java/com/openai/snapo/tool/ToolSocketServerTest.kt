...
class ToolSocketServerTest {
    @Test
    fun `disallowed startup never binds or reports a socket failure`() {
        ToolSocketServer(1, { error("Must not bind") }) {}.use { server ->
    ...
    fun `guarded startup does not hide programming errors`() {
        ToolSocketServer(1, { throw IllegalArgumentException("Invalid socket configuration") }) {}.use { server ->
    ...
    fun `connection limits reject excess clients without disturbing an active request`() {
        val listener = TcpListener()
        val entered = CountDownLatch(1)
        ToolSocketServer(1, { listener }) { connection ->
            entered.countDown()
    ...
    fun `failed bindings can be retried and closed servers can start a fresh session`() {
        ...
        ToolSocketServer(2, {
            ...
            val request = ToolHttpRequest.read(connection.input)
            if (request.path == "/fail") throw IOException("Synthetic handler failure")
            ToolHttpResponse(200, "hello".toByteArray()).write(connection.output)
        }.use { server ->
    ...
    fun `socket names follow the inspector discovery contract`() {
        assertEquals("snapo_example.inspector-v2_123", ToolSocketServer.socketName("example.inspector-v2", 123))
        for (id in listOf("", "Uppercase", "with_underscore", "a".repeat(101), "../example")) {
            assertThrows(IllegalArgumentException::class.java) { ToolSocketServer.socketName(id, 123) }
        }
        assertThrows(IllegalArgumentException::class.java) { ToolSocketServer.socketName("example", 0) }
    }

    private class TcpListener : ToolSocketListener {
        ...
        override fun accept(): ToolConnection = TcpConnection(socket.accept())
    }
