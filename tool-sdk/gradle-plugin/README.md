...
The implementation artifact is `com.openai.snapo:tool-packager-gradle-plugin`. Its group comes from `gradle.properties`, its name from this build's `settings.gradle.kts`, and its version from the root `VERSION`. The plugin ID is declared in `build.gradle.kts`. See [package release guidance](../../release/authoring.md) for publishing and renames.
