...
Bundled frontends do not need protocol-version configuration or checks. For independently shipped clients, set an explicit positive `protocolVersion`; only then does the plugin emit the descriptor field and `SnapOTool.PROTOCOL_VERSION`. Host API metadata is generated by the plugin and is not configurable.
