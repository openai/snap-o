...
```sh
python3 plugin-reader/build.py
python3 plugin-reader/build.py --check
```

The reproducible output is `plugin-reader/snapo-discovery.jar`. It is checked in so desktop and CLI users do not need Java or an Android SDK. Keep this file beside the standalone CLI when distributing it. The macOS app bundles it under `Contents/Resources`, outside the directory for signed executables. The reader adds no dependencies to inspected Android apps.
...
```sh
python3 plugin-reader/test-device.py --serial DEVICE --freeze --icon round
python3 plugin-reader/test-device.py --serial DEVICE --freeze --icon adaptive
python3 plugin-reader/test-device.py --serial DEVICE --freeze --icon legacy
```
