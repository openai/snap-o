...
export class NetworkConnection {
  ...
  async start(): Promise<void> {
    try {
      ...
      this.close(error instanceof Error ? error : new Error("Inspector connection failed."));
      throw error;
    }
  }

  close(error = new Error("Inspector connection ended.")): void {
  ...
  private checkOpen(): void {
    if (this.closed) throw new Error("Inspector is disconnected.");
  }
  ...
  private async json(path: string, limit: number, allowMissing = false): Promise<Record<string, unknown> | null> {
    ...
    if (!result || typeof result !== "object" || Array.isArray(result)) throw new Error("Invalid inspector response.");
  ...
  private openEvents(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => this.close(new Error("Inspector connection timed out.")), 5_000);
      this.stream = stream;
      const opened = () => {
      ...
      stream.addEventListener("error", () => {
        ...
        this.close(new Error("Inspector connection ended."));
      });
      stream.addEventListener("message", (event) => {
        ...
        try {
          ...
          if (event.lastEventId !== String(message.snapoSequence)) throw new Error("Invalid inspector event sequence.");
          if (this.replaying) {
            if (this.buffered.length >= 4096 || this.bufferedBytes + bytes > 32 * 1024 * 1024) {
              throw new Error("Inspector history could not keep up with live events.");
            }
            this.buffered.push(message);
            this.bufferedBytes += bytes;
          } else this.publishLive(message);
        } catch (error) {
          this.close(error instanceof Error ? error : new Error("Invalid inspector event."));
        }
      });
  ...
  private async history(): Promise<void> {
    ...
    if (
      ...
      await response.body?.cancel();
      throw new Error("Invalid inspector history response.");
    }
    ...
    try {
      while (true) {
        ...
        for (let index = 0; index < value.length; index++) {
          ...
          length += index - start;
          if (length > maximumRecordBytes) throw new Error("Inspector history record is too large.");
          line += decoder.decode(value.subarray(start, index));
          const message = parseRecord(line, length);
          if (message.snapoSequence! > this.watermark) throw new Error("Invalid inspector history sequence.");
      ...
      line += decoder.decode();
      if (length || line) throw new Error("Inspector history ended during a record.");
    } finally {
...
function parseRecord(text: string, bytes: number): CdpMessage {
  if (bytes > maximumRecordBytes) throw new Error("Inspector record is too large.");
